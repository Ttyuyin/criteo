...
# # ================= ⚙️ 消融实验开关 =================
# USE_MISSING_INDICATOR = True   # 优化点1：连续特征缺失/异常值指示特征 (_miss)
# USE_BINNING           = False     # 优化点2：连续特征分桶离散化 (_bin)
# USE_TARGET_ENCODING   = True    # 优化点3：交叉验证目标编码 (_target_enc)
# USE_SCALED_FM         = False   # 优化点4：ScaledDeepFM (自适应 field 权重)
...
USE_DNN_BN            = True   # 优化点5：DNN 是否使用 BatchNorm
...
# NUM 数值特征：清缺失负数 → 加缺失标识（可选）→ log1p 压长尾 → 分桶离散（可选）→ 归一化 → 参与目标编码。
# CAT 类别特征：填缺失 → 低频归为稀有类 → 目标编码增强（可选）→ 安全整数编码，供 Embedding 查表。
...
class ScaledFM(nn.Module):
    ...
    def forward(self, fm_input):
        # fm_input: [batch_size, field_num, embedding_dim]
        if fm_input.size(1) != self.n_fields:
            raise ValueError(
                f"ScaledFM field mismatch: fm_input has {fm_input.size(1)} fields, "
                f"but field_scale has {self.n_fields}."
            )
...
class ScaledDeepFM(BaseDeepFM):
    """真正替换父类 FM 模块的 ScaledDeepFM"""
    def __init__(self, linear_feature_columns, dnn_feature_columns, **kwargs):
        super().__init__(linear_feature_columns, dnn_feature_columns, **kwargs)

        # 只统计 SparseFeat，因为 FM 输入对应 embedding fields
        n_fields = sum(1 for feat in dnn_feature_columns if isinstance(feat, SparseFeat))

        # 关键：直接替换父类 self.fm，而不是定义同名方法
...
def main():
    ...
    for fold, (tr_idx, val_idx) in enumerate(outer_skf.split(train_raw_df, y_train_full)):
        ...
        # 构建给 LGBM 的输入（保持和历史实验口径一致：TE 继续喂给 LGBM）
        X_tr_lgb = tr_df.drop(['label', 'is_train'], axis=1)
        y_tr = tr_df['label'].values
        X_val_lgb = val_df.drop(['label', 'is_train'], axis=1)
        y_val = val_df['label'].values
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.798116
# Train OOF LogLoss : 0.449064
# -------------------------
# Test Score AUC    : 0.799212
# Test Score LogLoss: 0.443422
# ⏱️ 总耗时: 91.6 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.797811
# Train OOF LogLoss : 0.449076
# -------------------------
# Test Score AUC    : 0.798935
# Test Score LogLoss: 0.443606
# ⏱️ 总耗时: 91.0 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.797642
# Train OOF LogLoss : 0.449025
# -------------------------
# Test Score AUC    : 0.798954
# Test Score LogLoss: 0.443439
# ⏱️ 总耗时: 72.9 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.797923
# Train OOF LogLoss : 0.449056
# -------------------------
# Test Score AUC    : 0.798878
# Test Score LogLoss: 0.443582
# ⏱️ 总耗时: 70.8 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.797681
# Train OOF LogLoss : 0.449090
# -------------------------
# Test Score AUC    : 0.798820
# Test Score LogLoss: 0.443426
# ⏱️ 总耗时: 69.8 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.796252
# Train OOF LogLoss : 0.450539
# -------------------------
# Test Score AUC    : 0.797535
# Test Score LogLoss: 0.444605
# ⏱️ 总耗时: 36.9 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.796290
# Train OOF LogLoss : 0.450637
# -------------------------
# Test Score AUC    : 0.797411
# Test Score LogLoss: 0.444768
# ⏱️ 总耗时: 38.0 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.797632
# Train OOF LogLoss : 0.449230
# -------------------------
# Test Score AUC    : 0.797787
# Test Score LogLoss: 0.444435
# ⏱️ 总耗时: 41.3 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.796861
# Train OOF LogLoss : 0.449981
# -------------------------
# Test Score AUC    : 0.797901
# Test Score LogLoss: 0.444386
# ⏱️ 总耗时: 37.1 分钟
...
#SEED=2026
# USE_MISSING_INDICATOR = False   # 优化点1：连续特征缺失/异常值指示特征 (_miss)
# USE_BINNING           = True     # 优化点2：连续特征分桶离散化 (_bin)
# USE_TARGET_ENCODING   = True    # 优化点3：交叉验证目标编码 (_target_enc)
# USE_SCALED_FM         = False   # 优化点4：ScaledDeepFM (自适应 field 权重)
# USE_DNN_BN            = True    # 优化点5：DNN 是否使用 BatchNorm
# ============================================================
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.795970
# Train OOF LogLoss : 0.451025
# -------------------------
# Test Score AUC    : 0.798308
# Test Score LogLoss: 0.444174
# ⏱️ 总耗时: 70.0 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.797636
# Train OOF LogLoss : 0.449369
# -------------------------
# Test Score AUC    : 0.798842
# Test Score LogLoss: 0.443542
# ⏱️ 总耗时: 57.0 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.797707
# Train OOF LogLoss : 0.448891
# -------------------------
# Test Score AUC    : 0.798394
# Test Score LogLoss: 0.443908
# ⏱️ 总耗时: 45.6 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.796090
# Train OOF LogLoss : 0.450580
# -------------------------
# Test Score AUC    : 0.798530
# Test Score LogLoss: 0.443800
# ⏱️ 总耗时: 65.8 分钟
...
# 🔥 消融实验最终结果
# ============================================================
# Train OOF AUC     : 0.797829
# Train OOF LogLoss : 0.449039
# -------------------------
# Test Score AUC    : 0.798804
# Test Score LogLoss: 0.443528
# ⏱️ 总耗时: 71.3 分钟